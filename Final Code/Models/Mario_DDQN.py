...
class SkipFrame(gym.Wrapper):
    """Yuansong Feng, Suraj Subramanian, Howard Wang, Steven Guo.
        github:https://github.com/pytorch/tutorials/blob/master/intermediate_source/mario_rl_tutorial.py """
...
env = gym_super_mario_bros.make('SuperMarioBrosRandomStages-v1') # use random stages for training and standard for validating
env = JoypadSpace(
    env,
    [['right'],
    ['right', 'A']]
...
class DDQN_Agent:
    ...
    def __init__(self, observation_space, action_space, buffer_size, batch_size):
        ...
        self.gamma = 0.99 # discounts rewards
        self.tau = 1e-3 # slightly randomize the gradients
        self.learn_step = 0 # counter for when to update the target network
        self.update = 0 
    ...
    def act(self, state, epsilon):
        ...
        if np.random.random_sample() > epsilon: # DDDQN uses epilson greedy training, meaning it will take random actions in the begin of train to "explore"         
            self.q_network.eval()
    ...
    def learn(self, memory):
        ...
        state, action, reward, next_state, done = memory # pull saved states from the memory
        self.update += 1
...
Agent.load_weights("mario_ddqn_1m.pt")
train = True
...
def run(train, Agent, env):
    ...
    update_counter = 0
    if train:
        for i in tqdm(range(1, 5_000_01)):
...
Agent.save_weights("mario_ddqn_10m.pt")

rewards_filename = 'Rewards_10m.npy'
losses_filename = 'Losses_10m.npy'
